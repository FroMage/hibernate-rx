...
public class PgPoolExamples {
  ...
  @Test
  public void gettingStatrtetd(VertxTestContext testContext) {

    // Pool options
    PgPoolOptions options = new PgPoolOptions()
      .setPort(5432)
      .setHost("localhost")
      .setDatabase("hibernate-rx")
      .setUser("hibernate-rx")
      .setPassword("hibernate-rx")
      .setMaxSize(5);

    // Create the client pool
    PgPool client = PgClient.pool(options);

    BiConsumer<Object, Throwable> consumer = (o, r) -> {
      if (r != null) {
        System.out.println(r.toString());
        testContext.failNow((Throwable) r);
      } else {
    ...
    invokeQueryLater(client).whenComplete(consumer);
  }

  private CompletionStage<Object> invokeQueryLater(PgPool client) {//, final BiConsumer<Object, Throwable> consumer) {
	    // A simple query

...
public class DefaultRxPersistEventListener extends DefaultPersistEventListener implements RxPersistEventListener {
	...
	private void saveTransientEntity(RxPersistEvent event, Map createCache) {
		...
		if ( createCache.put( entity, entity ) == null ) {
			saveWithGeneratedId( entity, event.getEntityName(), createCache, source, false )
					.exceptionally( err -> {
						event.completeExceptionally( err );
						return null;
					} )
					.thenAccept( ignore -> {
						event.complete();
			...
			if ( LOG.isDebugEnabled() ) {
				LOG.debugf(
						...
						entityDescriptor.getIdentifierDescriptor()
								...
								.extractLoggableRepresentation( generatedId ),
		...
		if ( !useIdentityColumn ) {
			...
			if ( old != null ) {
				if ( source.getPersistenceContext().getEntry( old ).getStatus() == Status.DELETED ) {
					...
					performSaveStage.toCompletableFuture().completeExceptionally( new NonUniqueObjectException(
							id,
	...
	protected CompletionStage<Object> performSaveOrReplicate(
		...
		boolean shouldDelayIdentityInserts = shouldDelayIdentityInserts(
				requiresImmediateIdAccess,
				source,
				entityDescriptor
		);
...
//			}
		...
		EntityInsertAction insert = createInsertAction(
				values,
				id,
				entity,
				entityDescriptor,
		...
		entry.addExtraState( new CompletionStageExtraState( stage ) );
